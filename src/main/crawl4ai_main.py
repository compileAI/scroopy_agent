#!/usr/bin/env python3
"""
Crawl4AI scraper main entry point.
Processes all configured crawl4ai sources and writes results to Supabase.
"""
import asyncio
import sys
import logging
...
async def main_async(limit: Optional[int] = None, dry_run: bool = False, preview: int = 5):
    """Async main function for crawl4ai scraper."""
    logger.info("🚀 Starting crawl4ai scraper batch processing")
    
    try:
        # Run crawl4ai scraper
        articles = await run_crawl4ai_scraper(limit=limit)
...
def main():
    """Main entry point for crawl4ai scrapers."""
    import argparse
    
    parser = argparse.ArgumentParser(description="Run crawl4ai scrapers and write to Supabase")
